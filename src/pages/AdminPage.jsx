...
const Badge = ({ label, active, status }) => (
  <span
    className={`
      inline-flex items-center gap-1 px-4 py-1 rounded-full text-xs font-medium
            ...
            <div className="flex gap-2 pt-2">
              {data.status === 'Pending' && (
                <ActBtn
                  ...
                  onClick={() => onUpdate(data.id, 'Diproses')}
                />
              )}
              {data.status === 'Diproses' && (
                <ActBtn
                  ...
                  onClick={() => onUpdate(data.id, 'Selesai')}
                />
              )}
              {['Pending', 'Diproses'].includes(data.status) && (
                <ActBtn
                  label="Batal"
                  colorCls="bg-red-100 text-red-500"
                  onClick={() => onUpdate(data.id, 'Dibatalkan')}
                />
              )}
            </div>
          </motion.div>
...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      snap => {
        ...
        /* 🔔 suara + toast jika pending bertambah */
  ...
  /* aksi update status (toast.promise) */
    ...
    <main className="min-h-screen px-4 md:px-8 py-10 space-y-6" style={{ background: BASE_BG }}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="flex items-center gap-2 text-xl font-bold text-[#363636]">
          ...
          {!!pendingCount && (
            <span
              className="ml-2 px-2 py-[2px] rounded-full text-[10px] font-semibold text-white"
      ...
      <div className="relative">
        ...
        <input
          ...
          className="
            ...
            bg-white/80 backdrop-blur border border-[#E5E5E5]
            focus:outline-none focus:ring-2 focus:ring-[#FE724C]/40
          "
        />
      </div>
